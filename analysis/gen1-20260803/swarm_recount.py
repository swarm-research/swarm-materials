...
def colony(root, native_only):
    ...
    out = Counter()
    unattributable = Counter()
    for kind in KINDS:
        d = root / "commons" / kind
        if not d.is_dir():
            continue
        # rglob, not iterdir: 566 gen-2 artifacts live inside challenge
        # subdirectories, and a top-level walk cannot see them. Two agents
        # documented this same blind spot independently.
        for f in d.rglob("*"):
            if not f.is_file():
                continue
            n = agent_num(f.name)
            if n is None:
                unattributable[kind] += 1   # 27.4% of gen 1 lands here
                continue
    ...
    for m in jsonl(root / "board" / "messages.jsonl"):
        ...
        if native_only and n <= GEN2_CUTOFF:
            continue
        msgs += 1
    return out, cites, junk, inherited, msgs, unattributable
...
def main():
    rows = []
    o, c, j, inh, m, u = colony(GEN1, native_only=False)
    rows.append(("gen1", o, c, j, inh, m, u))

    for d in sorted(GEN2.iterdir()):
        if d.name.startswith(".") or not (d / "swarm").is_dir():
            continue
        o, c, j, inh, m, u = colony(d / "swarm", native_only=True)
        rows.append((f"gen2·{d.name}", o, c, j, inh, m, u))
    ...
    g2o = g2c = g2m = 0
    for name, o, c, j, inh, m, u in rows:
        tot = sum(o.values())
    ...
    unattr = sum(sum(r[6].values()) for r in rows)
    print(f"\n无法按文件名归属的文件：{unattr} —— 这些不在上面任何一列里，"
          f"所以每个产出数都是下界")
